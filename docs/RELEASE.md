...
- Docker image name: `securestore:1.0.1`
...
3. Run `npm run build`.
4. Build the container with `docker build -t securestore:1.0.1 .`.
5. Commit all files.
6. Create the annotated tag with `git tag -a v1.0.1 -m "SecureStore"`.
...
```bash
gh release create v1.0.1 --title "SecureStore" --notes-file CHANGELOG.md
```
